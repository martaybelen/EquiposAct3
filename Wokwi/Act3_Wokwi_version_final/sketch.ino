...
void loop() {
  ...
  if (modo == 1) { // Modo 1: Humedad y Temperatura
    ...
    if(temperatura_anterior != temperatura || humedad_anterior != humedad){
      ...
      //Se muestra la temperatura y humedad medida del DHT en el LCD
      lcd.display();
      lcd.backlight();
      LimpiezaLCD();
  ...
  if (modo == 3) { // Modo 3: Presencia
    ...
    if (distancia > distanciaanterior + 3 || distancia < distanciaanterior - 3) {
      ...
      //La medida se encuentra en el rango de distancias de este sensor
        ...
        //En caso de fuera de rango se informa de ello en el LCD y no se enciende el led
        lcd.setCursor(0, 0);
        lcd.write(9);
        lcd.setCursor(2, 0);
  ...
  //Entramos en modo apagado, el led RGB y la patalla del LCD se apagan
...
//Función para medir la distancia a través del sensor en el modo 3
long medirDistancia() {
  digitalWrite(PINTRIG, LOW);
  delayMicroseconds(2);
...
//Método para mostrar las temperaturas en el modo 4
...
//Método para encender los ldes en el modo de ilumianción de acuerdo con la lógica difusa
...
//Función para mostrar en la pantalla LCD la iluminación
void mostrarIluminacion(float lux, float lux_anterior) {

//Solo en caso de que cambie lux cambia el valor del LCD
  if (lux_anterior != lux) {
    lcd.display();
    lcd.backlight();
    LimpiezaLCD();
    ...
    lcd.print(lux, 1);
    if(modoControl==0){
      //Se muestra en forma de porcentaje en el caso de estar en el modo iluminación al 80%
      lcd.print(" %");
    }
  }
}

//Método para el control de la iluminación al 80%
